...
This branch prepares the MIT-licensed `0.3.2` terminal-presentation maintenance
artifact from exact source commit `ed271757daf80c3ded7ae2b4a67d74102ebf2435`.
Its terminal layout preserves the baseline's runtime, workspace, and persistence
implementation. The package remains `private: true` and is not published to npm.
Hamilton ADR 0032 selects a separately identified, hash-verified repository-retained
artifact after source-delta and consumer validation. Hamilton ADR 0033 additionally selects the exact
`debugger-sh@0.3.15-webide.legacy.1` immutable fork asset for the minimal
empty-stdin-read correction. Its source, artifact, embedded Wasm, and build
identities are recorded in `release/maintenance-engine-input.json`. No other
runtime feature or persistence change is included.

The retained release scripts/configuration describe historical `0.3.1`; they do
not generate a `0.3.2` release or maintenance receipt. This maintenance artifact
requires its own exact source, patch, package hash, license, dependency, and
browser evidence retained by its consumer.
The current built-in browser runtime providers support
