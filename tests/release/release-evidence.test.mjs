...
describe('bounded subprocess evidence', () => {
  ...
  it('normalizes completed path-bearing logs and preserves the exact final receipt', async () => {
    ...
    const raw = Buffer.concat([
      splitRepositoryPath,
      Buffer.from([
        `candidate: ${candidate}/web-ide-0.4.0.tgz`,
    ...
    expect(normalized).toContain('candidate: <web-candidate>/web-ide-0.4.0.tgz')
...
describe('artifact manifest', () => {
  it('binds the exact candidate inventory, source/tag, runtime, build inputs, logs, and strict schema', async () => {
    ...
    await Promise.all([
      writeFile(path.join(directory, 'web-ide-0.4.0.tgz'), candidateBytes),
      writeFile(path.join(directory, 'web-ide-0.4.0-source.tar.gz'), sourceBytes),
    ...
    const manifest = await createArtifactManifest({
      ...
      source: {
        ...
        commitTimestamp: 1,
        sourceDateEpoch: '1',
        tag: { name: 'web-ide-v0.4.0-source-r3', objectId: 'd'.repeat(40), objectType: 'tag', peeledCommit: sourceCommit },
...
describe('release source state', () => {
  it('independently binds GitHub branch, annotated tag object, and peeled commit', async () => {
    ...
    const responses = new Map([
      ...
      ['https://api.github.com/repos/justinvassantachart/web-ide/git/ref/tags/web-ide-v0.4.0-source-r3', {
        ref: 'refs/tags/web-ide-v0.4.0-source-r3',
    ...
    await expect(verifyIndependentGitHubSource(
      { sourceRepository, sourceTag: 'web-ide-v0.4.0-source-r3' },
      { commit, tagObjectId },
      fakeFetch,
    )).resolves.toEqual({ branchCommit: commit, tagObjectId, peeledCommit: commit })
    expect(fetched).toEqual([...responses.keys()])

    responses.get('https://api.github.com/repos/justinvassantachart/web-ide/git/ref/tags/web-ide-v0.4.0-source-r3')
      .object.type = 'commit'
    await expect(verifyIndependentGitHubSource(
      { sourceRepository, sourceTag: 'web-ide-v0.4.0-source-r3' },
      { commit, tagObjectId },
  ...
  it('requires a clean pushed main and pushed source tag, and makes a deterministic local archive', async () => {
    ...
    await run('git', ['tag', '-a', 'web-ide-v0.4.0-source-r3', '-m', 'Web IDE 0.4.0 fixture'], {
      cwd: checkout,
      env: gitIdentityEnvironment,
    })
    await run('git', ['push', 'origin', 'main', 'refs/tags/web-ide-v0.4.0-source-r3'], { cwd: checkout })
    const configuration = {
    ...
    expect(source).toMatchObject({ branch: 'main', tag: { name: 'web-ide-v0.4.0-source-r3', objectType: 'tag' } })
